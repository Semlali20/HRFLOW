...
public class NotificationServiceImpl implements NotificationService {
        ...
        try {
            ...
            collaborateurRepository.findAll().forEach(collaborateur -> {
                try {
                    ...
                    log.error("Error processing birthday notification for collaborateur id={}: {}",
                            collaborateur.getMatricule(), e.getMessage(), e);
                }
            });
            ...
            stagiaireRepository.findAll().forEach(stagiaire -> {
                try {
                    ...
                    log.error("Error processing meeting notification for stagiaire id={}: {}",
                            stagiaire.getMatricule(), e.getMessage(), e);
                }
            });

            log.info("Daily notifications completed");
        } catch (Exception e) {
            log.error("Fatal error during daily notification job: {}", e.getMessage(), e);
        }
    }
    ...
    public Notification push(User user, String title, String message, NotificationType type) {
        try {
            ...
            log.error("Failed to push notification to user '{}': {}", user.getEmail(), e.getMessage(), e);
            throw new RuntimeException("Failed to push notification: " + e.getMessage(), e);
        }
    }
    ...
    public List<Notification> getUnread(String email) {
        try {
            ...
            log.error("Failed to fetch unread notifications for '{}': {}", email, e.getMessage(), e);
    ...
    public void markAllRead(String email) {
        try {
            ...
            log.debug("Marked {} notifications as read for '{}'", unread.size(), email);
        } catch (Exception e) {
            log.error("Failed to mark notifications as read for '{}': {}", email, e.getMessage(), e);
    ...
    private void sendBirthdayEmailToCollaborateur(Collaborateurs collaborateur) {
        try {
            ...
            vars.put("nom", collaborateur.getNom());
            vars.put("prenom", collaborateur.getPrenom());
            ...
            log.error("Failed to send birthday email to '{}': {}", collaborateur.getEmail(), e.getMessage(), e);
        }
    }
}
